<tr>
    ...
    <td x-show="employeeID" x-transition x-cloak>{{ $clubReservations->user->employeeID }}</td>
    <td x-show="PrimaryUserFullName" x-transition>{{ $clubReservations->user->name . ' ' . $clubReservations->user->lastName }}</td>
    <td x-show="primaryUserNationalCode" x-transition x-cloak>{{ $clubReservations->user->nationalCode }}</td>
    <td x-show="clubName" x-transition x-cloak>{{ $clubReservations->club->clubName }}</td>
    <td x-show="genderSpecific" x-transition x-cloak>{{ $clubReservations->club->genderSpecific }}</td>
    ...
    <td x-show="secondaryUserNationalCode" x-transition x-cloak>{{ $clubReservations->secondayUserNationalCode }}</td>
    ...
    <td x-show="employmentTypeName" x-transition x-cloak>{{ $clubReservations->user->employmentType->employmentTypeName }}</td>
    <td x-show="unitName" x-transition x-cloak>{{ $clubReservations->user->unit->unitName }}</td>
