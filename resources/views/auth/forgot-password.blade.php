          ...
          <div class="card-body">
            ...
            <div class="app-brand justify-content-center">
              <a href="/" class="app-brand-link gap-2">
                ...
                <span class="app-brand-text demo h3 mb-0 fw-bold">بازیابی کلمه عبور</span>
              </a>
            </div>
            <!-- /Logo -->
            @if(session()->has('failed'))
              <div class="text-red">{{ session('failed') }}</div>
            @endif
            <h4 class="mb-3 secondary-font">تاییدیه پیامکی</h4>
            <p class="mb-0 fw-semibold">لطفا شماره موبایل خود را وارد نمایید</p>
            <form id="twoStepsForm" action="{{ route('password.mobile') }}" method="POST">
                @csrf
              <div class="mb-4">
                
                <input  type="text" id="mobile" name="mobileNumber" placeholder="شماره موبایل" class="form-control text-start">
                <span class="invalid-feedback" id="invalid"></span>
                @error('mobileNumber')
                  <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
			  <input type="submit" value="ارسال کد تایید" class="btn btn-primary d-grid w-100 mb-3">
