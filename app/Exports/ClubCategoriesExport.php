...
class ClubCategoriesExport implements FromQuery, withHeadings, shouldAutoSize, withMapping, withEvents, WithStyles
{
    use Exportable;
    ...
    {
        return DB::table('clubcategory')
            ->select(
                'clubCategoryID',
                'categoryName',
            )
            ->orderBy('clubCategoryID')
            ->when($this->ids, function ($query) {
                $query->whereIn('clubCategoryID', $this->ids);
            });
    }
